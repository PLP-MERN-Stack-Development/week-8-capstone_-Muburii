...
const SignupTeacher = async (req, res) => {
  ...
  try {
    ...
    if (await User.findOne({ email })) {
      return res.status(400).json({ message: 'Email already Signuped' });
    }
    ...
    res.status(201).json({ message: 'Teacher Signuped successfully' });
  } catch (err) {
...
module.exports = { login, SignupTeacher, getMe };